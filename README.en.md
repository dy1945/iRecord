...
A native macOS screen recorder and screenshot tool built with Swift.

iRecord uses Apple's native capture stack so captured GPU surfaces flow straight
...
- **Pin (贴图)** — floats the shot always-on-top: drag to move, scroll to
  ...
  on-screen pixels, they can be included in a new screenshot.
