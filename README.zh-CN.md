...
在录屏之外，iRecord 现在内置了一套对标 iShot 的截图功能：

- **区域截图** —— 先冻结所有显示器，然后提供全屏十字参考线、**放大镜**（像素放大 + `#HEX` 色值 + 坐标）和实时 `宽 × 高` 尺寸显示。拖拽完成后，悬浮工具条提供：**长截图 · 贴图 · 编辑 · 复制 · 保存 · 取消**。
  按键：**Enter** 复制 · **Space** 保存 · **S** 长截图 · **T** 贴图 · **R / H** 复制像素 RGB / HEX 色值 · **Esc / 右键** 取消 · 双击立即复制。
...
- **标注编辑器（截屏编辑）** —— 画笔 · 直线 · 箭头 · 矩形 · 椭圆 · **马赛克**（像素块）· 文字 · 局部高亮；5 个预设色 + 自定义取色、3 档线宽、⌘Z 撤销。输出为合成后的图片，可保存 / 复制 / 贴图。
- **贴图（Pin）** —— 截图置顶悬浮：拖动移动、滚轮缩放、悬停显示关闭按钮与透明度滑条、右键菜单（标注 / 复制 / 保存 / 关闭）、双击或 Esc 关闭。贴图是普通的屏幕像素，所以下一次截图会把它们拍进去 —— 即 iShot 的**二次截屏**。右键 ▸ 标注会重新打开编辑器并把结果写回贴图（**二次标注**）。另有快捷键直接贴出剪贴板图片，以及一键隐藏 / 显示所有贴图。
...
> 有意**排除**的内容：iRecord 的插件架构（这是项目目标决定的）。
