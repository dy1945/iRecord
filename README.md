...
**直接安装与升级提示：** 当前 GitHub 下载包采用 ad-hoc 签名，尚未经过 Apple 公证。直接安装或覆盖升级后，macOS 可能要求重新授予屏幕录制权限；使用麦克风时，也可能需要重新授权麦克风。如果此前已授权但新版仍提示缺少权限，请退出 iRecord，在「系统设置 → 隐私与安全性」中检查对应权限项，重新为 iRecord 开启权限，再启动应用。是否需要重新授权由 macOS 判断，并非每次启动都需要。
